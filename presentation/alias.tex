...
\begin{frame}[fragile]
  ...
  \begin{codeblock}{Proposal: Expression alias}
    \begin{lstlisting}
namespace Y
{
  using bar = (X::foo);
  ...
  \begin{block}{Counterargument}
    Does not disambiguate between function, templates and variables.
    Parens required to alias anything other than type.
  \end{block}
\end{frame}
